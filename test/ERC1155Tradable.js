...
describe("ERC1155Tradable - ERC 1155", () => {
  before(async () => {
    ...
    /* Defining Accounts */
    [owner, creator, userA, userB, proxyForOwner, ..._others] =
  ...
  describe("#create()", () => {
    it("should allow the contract owner to create tokens with zero supply", async () => {
      tokenId += 1;
      await expect(
        instance.connect(owner).create(owner.address, tokenId, 0, "", "0x00")
      )
    ...
    it("should allow the contract owner to create tokens with initial supply", async () => {
      tokenId += 1;
      await expect(
        instance.connect(owner).create(owner.address, tokenId, MINT_AMOUNT, "", "0x00")
      )
        .to.emit(instance, "TransferSingle")
        .withArgs(
          owner.address,
    ...
    it("should set tokenSupply on creation", async () => {
      ...
      await expect(
        instance.connect(owner).create(owner.address, tokenId, tokenSupply, "", "0x00")
      )
        .to.emit(instance, "TransferSingle")
        .withArgs(
          owner.address,
    ...
    it("should not allow a non-owner to create tokens", async () => {
      tokenId += 1;
      await expect(
        instance.connect(userA).create(userA.address, tokenId, 0, "", "0x00")
      ).to.be.revertedWith("caller is not the owner");
    });

    it("should allow the contract owner to create tokens and emit a URI", async () => {
      tokenId += 1;
      await expect(
        instance.connect(owner).create(owner.address, tokenId, 0, vals.URI_BASE, "0x00")
      )
        .to.emit(instance, "URI")
        .withArgs(vals.URI_BASE, toBN(tokenId));
    });

    it("should not emit a URI if none is passed", async () => {
      tokenId += 1;
      await expect(
        instance.connect(owner).create(owner.address, tokenId, 0, "", "0x00")
  ...
  describe("#totalSupply()", () => {
    it("should return correct value for token supply", async () => {
      tokenId += 1;
      await instance
        .connect(owner)
    ...
    it("should return zero for non-existent token", async () => {
      ...
      const supplyAccessorValue = await instance.totalSupply(NON_EXISTENT_TOKEN_ID);
      assert.ok(supplyAccessorValue.eq(toBN(0)));
    });
  });

  describe("#setCreator()", () => {
    it("should allow the token creator to set creator to another address", async () => {
    ...
    it("should allow the new creator to set creator to another address", async () => {
    ...
    it("should not allow the token creator to set creator to 0x0", () => {
      expect(
        instance.connect(creator).setCreator(vals.ADDRESS_ZERO, [INITIAL_TOKEN_ID])
      ).to.be.revertedWith("ERC1155Tradable#setCreator: INVALID_ADDRESS.");
    });

    it("should not allow a non-token-creator to set creator", async () => {
      // Check both a user and the owner of the contract
      await expect(
        instance.connect(userA).setCreator(userA.address, [INITIAL_TOKEN_ID])
      ).to.be.revertedWith("ERC1155Tradable#creatorOnly: ONLY_CREATOR_ALLOWED");
      await expect(
        instance.connect(owner).setCreator(owner.address, [INITIAL_TOKEN_ID])
      ).to.be.revertedWith("ERC1155Tradable#creatorOnly: ONLY_CREATOR_ALLOWED");
    });
  });
  ...
  describe("#mint()", () => {
    ...
    it("should not overflow token balances", async () => {
      const supply = await instance.totalSupply(INITIAL_TOKEN_ID);
  ...
  describe("#batchMint()", () => {
    ...
    it("should not overflow token balances", () => {
      return expect(
        instance
          .connect(creator)
          .batchMint(userA.address, [INITIAL_TOKEN_ID], [OVERFLOW_NUMBER], "0x00")
      ).to.be.reverted;
    });
  ...
  describe("#uri()", () => {
    ...
    it("should not return the uri for a non-existent token", async () => {
      expect(instance.uri(NON_EXISTENT_TOKEN_ID)).to.be.revertedWith(
        "NONEXISTENT_TOKEN"
      );
    });
  });
  ...
  describe("#setURI()", () => {
    ...
    it("should not allow non-owner to set the url", async () => {
      expect(instance.connect(userA).setURI(newUri)).to.be.revertedWith(
  ...
  describe("#isApprovedForAll()", () => {
    ...
    it("should not approve non-proxy address as _operator", async () => {
      assert.isNotOk(await instance.isApprovedForAll(owner.address, userB.address));
    });
  ...
  describe("#executeMetaTransaction()", () => {
    it("should allow calling setApprovalForAll with a meta transaction", async () => {
      ...
      const eventEmitted = (receipt, eventSig, time) => {
        ...
        const parsed = evts.map((x) => ERC1155Interface.parseLog(x));
        return [evts.length === time, parsed];
      };

      const functionSignature = ERC1155Interface.encodeFunctionData(
        "setApprovalForAll",
      ...
      const tx = await instance.executeMetaTransaction(
        user,
        functionSignature,
        r,
        s,
      ...
      const [emitted, parsed] = eventEmitted(
        receipt,
